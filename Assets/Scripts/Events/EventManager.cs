    ...
    public static EventManager instance
    {
        ...
        public static void StartListening<Tbase> (UnityAction listener) where Tbase : UnityEvent, new()
        {
            ...
            if (instance.eventDictionary.TryGetValue(typeof(Tbase), out thisEvent))
            {
                ...
                if (e != null) e.AddListener(listener);
                else Debug.LogError ("EventManager.StartListening() FAILED! Event type " + typeof(Tbase).ToString() + " could not be accessed for some strange reason.");
        ...
        public static void StopListening<Tbase, T0> (UnityAction<T0> listener) where Tbase : UnityEvent<T0>
        {
            ...
            {
                Tbase e = thisEvent as Tbase;
                if (e != null) e.RemoveListener(listener);
                else Debug.LogError ("EventManager.StopListening() FAILED! Event type " + typeof(Tbase).ToString() + " could not be accessed for some strange reason.");
            }
        }
        ...
        public static void TriggerEvent<Tbase>() where Tbase : UnityEvent
        {
            ...
            if (instance.eventDictionary.TryGetValue(typeof(Tbase), out thisEvent))
            {
                ...
                if (e != null) e.Invoke();
                else Debug.LogError ("EventManager.TriggerEvent() FAILED! Event type " + typeof(Tbase).ToString() + " could not be accessed for some strange reason.");
            }
        }
